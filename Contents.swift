...
struct Sword: Item {
    ...
    let effect: (GameCharacter) -> Void = { character in guard let warrior = character as? Warrior else { return }
        warrior.strength *= 2
    }
}
...
struct Potion: Item {
    ...
    let effect: (GameCharacter) -> Void = { character in character.heal(amount: 50)
    }
}
...
mage.printCharacterInfo()     // Информация о маге после атаки

// Использование предмета Меч
print("\nИспользование предмета Меч:")
let sword = Sword() // Создаем экземпляр меча
sword.effect(warrior) // Применяем эффект меча к воину
warrior.printCharacterInfo() // Проверяем силу воина после использования меча

// Атака воина на мага после использования меча
print("\nВоин атакует мага после использования меча:")
warrior.attack(target: mage)
mage.printCharacterInfo()     // Информация о маге после повторной атаки

// Лечение мага
print("\nМаг лечит себя:")
mage.healTarget(target: mage)
mage.printCharacterInfo() // Информация о маге после лечения

// Использование предмета Зелье
print("\nИспользование предмета Зелье:")
let potion = Potion() // Создаем экземпляр зелья
potion.effect(mage) // Применяем эффект зелья к магу
mage.printCharacterInfo() // Проверяем здоровье мага после использования зелья

// Полёт мага
print("\nМаг пытается полететь:")
mage.fly(duration: 30) // Маг летит 30 секунд
mage.printCharacterInfo() // Информация о маге после полета
